    ...
    def __call__(self, *args, **kwds):
        ...
        for listener in self.task.get_listeners():
            if listener['filter'].filter(self.method, *args, **kwds):
                listener['task'].get_queue().put(
                    DistributedMethod(self.task, self.method, NoFuture(), *args, **kwds)
                )
...
class Manager(object):
    ...
    def load_modules(self, task_list, daemon=0):
        self.state = "Loading"
        index = 0
    ...
    def dump_modules(self):
        template = "{0}\t{1}\t{2}"
        print(template.format("Actor", "State", "Queue"))
        for name, module in list(self.modules.items()):
            print(
                template.format(name, module.runner.state, module.runner.queue.qsize())
            )
